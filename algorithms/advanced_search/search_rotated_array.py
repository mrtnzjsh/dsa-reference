"""
Search in Rotated Sorted Array Algorithm Implementation

This module implements the search algorithm for finding a target element in a rotated sorted array.
A rotated sorted array is a sorted array that has been rotated at some pivot point, meaning the
array has been shifted such that the elements are no longer in their original order, but still
maintain a sorted property. The algorithm uses a modified binary search approach to efficiently
locate the target in O(log n) time.
...
Note:
    This algorithm assumes the array contains distinct elements (no duplicates).
    With duplicates present, worst-case time complexity can degrade to O(n).
    The key insight is that at least one half of the array [low, mid] or [mid, high]
    will always be sorted in a rotated sorted array, allowing binary search to be
    used effectively without knowing the rotation point in advance.
"""
