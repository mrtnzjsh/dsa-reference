...
This module implements the exponential search algorithm, which efficiently finds a target element
in a sorted array by exponentially increasing the search range. The algorithm combines the
exponential technique with binary search to achieve optimal performance for sorted datasets
where the target might be near the beginning or the end of the array.
...
Note:
    Exponential search is particularly useful for unbounded or very large arrays
    where the index of the target is not known in advance. It can also be used
    with infinite sorted sequences when the target is likely to be near the beginning.
    When the target is near the beginning, exponential search is more efficient
    than traditional binary search which always starts from the middle.
"""
