...
ALGORITHM OVERVIEW:
A Binary Search Tree (BST) is valid if it satisfies the BST property:
...
- The property must hold for all nodes in the tree

MATHEMATICAL FOUNDATION:
Formally, a binary tree with root r is a BST if:
1. The left subtree of r contains only nodes with values less than r.val
2. The right subtree of r contains only nodes with values greater than r.val
3. Both left and right subtrees are also BSTs

The validation can be performed using a bottom-up approach or a top-down
approach with range validation.

TIME COMPLEXITY: O(n) where n is the number of nodes
SPACE COMPLEXITY: O(h) for recursion stack (O(log n) for balanced BST, O(n) for skewed)

EDGE CASES:
- Empty tree: Considered valid BST
- Single node: Always valid
- Duplicate values: Violates BST property (strict inequality)
- Negative numbers: Valid in BST (no restriction on value range)
- Very large trees: Handle with recursion depth considerations

TRADE-OFFS:
- Simple validation: O(n) time, uses min/max bounds
- With parent pointers: Can be O(n) but with different strategy
- AVL/RB tree validation: More complex with balance checks
